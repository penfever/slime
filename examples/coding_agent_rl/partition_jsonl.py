...
def partition_jsonl(input_path: Path, train_path: Path, eval_path: Path, *, eval_size: int, seed: int) -> None:
    ...
    try:
        with train_temp, eval_temp:
            for index, line in enumerate(_nonempty_lines(input_path)):
                destination = eval_temp if index in eval_indices else train_temp
...
def main() -> None:
    ...
    partition_jsonl(args.input, args.train, args.eval, eval_size=args.eval_size, seed=args.seed)
